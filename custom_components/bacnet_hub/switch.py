...
from .published_point_entities import BacnetPublishedSwitchObserver
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities) -> None:
    ...
    _setup_client_point_platform(
        ...
        entry,
        async_add_entities,
        match=lambda point: _point_platform(point) == "switch",
        build=lambda client_id, client_instance, point_key, point: BacnetClientPointSwitch(
            hass=hass,
            entry_id=entry.entry_id,
            client_id=client_id,
